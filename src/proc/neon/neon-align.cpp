...
namespace librealsense
{
    ...
    static inline void distorte_x_y(
        ...
        float32x4_t *distorted_x, float32x4_t *distorted_y, const rs2_intrinsics &_)
    {
        *distorted_x = x;
        *distorted_y = y;
    }
    ...
    inline void distorte_x_y<RS2_DISTORTION_MODIFIED_BROWN_CONRADY>(
        const float32x4_t &x, const float32x4_t &y,
        float32x4_t *distorted_x, float32x4_t *distorted_y, const rs2_intrinsics &to)
    {
        float32x4_t c[5];
        for (int i = 0; i < 5; ++i)
        {
        ...
        const auto x_f_y_f = vmulq_f32(x_f, y_f);

        // dx = x_f + 2 * c[2] * x_f * y_f + c[3] * (r2 + 2 * x_f * x_f)
        const auto tmp_x = vmulq_f32(c[3], vaddq_f32(r2, vmulq_f32(two, vmulq_f32(x_f, x_f))));
        const auto dx = vaddq_f32(x_f, vaddq_f32(vmulq_f32(c[2], vmulq_f32(two, x_f_y_f)), tmp_x));

        // dy = y_f + 2 * c[3] * x_f * y_f + c[2] * (r2 + 2 * y_f * y_f)
        const auto tmp_y = vmulq_f32(c[2], vaddq_f32(r2, vmulq_f32(two, vmulq_f32(y_f, y_f))));
        const auto dy = vaddq_f32(y_f, vaddq_f32(vmulq_f32(c[3], vmulq_f32(two, x_f_y_f)), tmp_y));
